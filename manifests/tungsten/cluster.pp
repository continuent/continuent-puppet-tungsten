# == Class: tungsten::tungsten::cluster See README.md for documentation.
#
	...
	if $location == true {
	  ...
	  case fileextension($location) {
	    ...
	    ".gz": {
	      ...
	      # Unpack the continuent-tungsten package from the provided location
	      Class["tungsten::prereq"] ->
    		exec { "unpack-continuent-tungsten":
    		  path => ["/bin", "/usr/bin"],
    		...
    		exec { "install-continuent-tungsten":
    		  path => ["/bin", "/usr/bin"],
    	...
    	default: {
    	  fail("Unable to install Continuent Tungsten from $location because the file type is not recognized.")
    	}
	  }
	}
}